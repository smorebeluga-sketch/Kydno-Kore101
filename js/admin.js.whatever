<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>admin.js container</title>
</head>
<body>

<script>
// =====================================
// ADMIN — CANCEL ANY WAITING FLIP
// =====================================

async function adminCancelWaitingFlip(flipId, button) {
    if (!flipId) return;

    try {
        const isAdmin = localStorage.getItem("kydnoIsAdmin") === "true";

        if (!isAdmin) {
            alert("Admin access required.");
            return;
        }

        const confirmed = confirm(
            "Are you sure you want to cancel this flip?"
        );

        if (!confirmed) return;

        button.disabled = true;
        button.textContent = "Cancelling...";

        const { data, error } = await supabaseClient.rpc(
            "admin_cancel_flip",
            { p_flip_id: flipId }
        );

        if (error) {
            console.error("Admin cancel flip error:", error);
            alert("Unable to cancel flip:\n\n" + error.message);
            button.disabled = false;
            button.textContent = "Cancel Flip";
            return;
        }

        if (data !== true) {
            alert("The flip could not be cancelled.");
            button.disabled = false;
            button.textContent = "Cancel Flip";
            return;
        }

        if (typeof loadActiveFlips === "function") {
            await loadActiveFlips();
        }
    } catch (error) {
        console.error("Admin cancel flip error:", error);
        alert(error.message || "Unable to cancel flip.");
        button.disabled = false;
        button.textContent = "Cancel Flip";
    }
}

function addAdminCancelButtons() {
    if (localStorage.getItem("kydnoIsAdmin") !== "true") return;

    const flipCards = document.querySelectorAll(".active-flip-card");

    flipCards.forEach((card) => {
        if (card.querySelector(".admin-cancel-flip-button")) return;

        const joinButton = card.querySelector(".join-active-flip-button");
        const existingCancelButton =
            card.querySelector(".cancel-active-flip-button");

        const flipId =
            joinButton?.dataset.flipId ||
            existingCancelButton?.dataset.flipId;

        if (!flipId) return;

        const button = document.createElement("button");
        button.type = "button";
        button.className = "admin-cancel-flip-button";
        button.textContent = "Cancel Flip";
        button.dataset.flipId = flipId;

        button.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
            adminCancelWaitingFlip(flipId, button);
        });

        card.appendChild(button);
    });
}

const adminFlipObserver = new MutationObserver(() => {
    addAdminCancelButtons();
});

const adminFlipList = document.getElementById("active-flips-list");

if (adminFlipList) {
    adminFlipObserver.observe(adminFlipList, {
        childList: true,
        subtree: true
    });

    addAdminCancelButtons();
}

// =====================================
// KYDNO KORE — ADMIN INVENTORY (-1 ONLY)
// =====================================

(function () {
    "use strict";

    const inventoryContent =
        document.getElementById("admin-inventory-content");

    if (!inventoryContent) {
        console.error("Admin inventory content not found.");
        return;
    }

    function getPetImage(itemName) {
        if (
            typeof itemImages !== "undefined" &&
            itemImages &&
            itemImages[itemName]
        ) {
            return itemImages[itemName];
        }

        return "";
    }

    function escapeHtml(value) {
        return String(value)
            .replace(/&/g, "&amp;")
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")
            .replace(/"/g, "&quot;")
            .replace(/'/g, "&#039;");
    }

    window.renderAdminPetInventory = function (inventory) {
        if (!Array.isArray(inventory) || inventory.length === 0) {
            inventoryContent.innerHTML = `
                <div class="admin-inventory-empty">
                    No Items Found
                </div>
            `;
            return;
        }

        const groupedPets = {};

        inventory.forEach((item) => {
            const petName = item.item_name;

            if (!groupedPets[petName]) {
                groupedPets[petName] = {
                    name: petName,
                    count: 0
                };
            }

            groupedPets[petName].count++;
        });

        inventoryContent.innerHTML = Object.values(groupedPets)
            .map((pet) => {
                const image = getPetImage(pet.name);

                return `
                    <div
                        class="admin-pet-inventory-row"
                        data-item-name="${escapeHtml(pet.name)}"
                    >
                        <div class="admin-pet-image-container">
                            ${
                                image
                                    ? `
                                        <img
                                            src="${image}"
                                            alt="${escapeHtml(pet.name)}"
                                            class="admin-pet-image"
                                        >
                                    `
                                    : `
                                        <div class="admin-pet-image-placeholder">
                                            ?
                                        </div>
                                    `
                            }
                        </div>

                        <div class="admin-pet-name">
                            ${escapeHtml(pet.name)}
                        </div>

                        <div class="admin-pet-controls">
                            <button
                                type="button"
                                class="admin-pet-minus"
                                data-item-name="${escapeHtml(pet.name)}"
                            >
                                −
                            </button>

                            <span class="admin-pet-count">
                                ${pet.count}
                            </span>
                        </div>
                    </div>
                `;
            })
            .join("");

        attachInventoryButtons();
    };

    function attachInventoryButtons() {
        document.querySelectorAll(".admin-pet-minus").forEach((button) => {
            button.addEventListener("click", async function () {
                if (
                    typeof adminSelectedUser === "undefined" ||
                    !adminSelectedUser
                ) {
                    alert("No user selected.");
                    return;
                }

                const row = button.closest(".admin-pet-inventory-row");
                const countElement =
                    row?.querySelector(".admin-pet-count");

                if (!row || !countElement) return;

                const itemName = button.dataset.itemName;
                const currentCount =
                    Number(countElement.textContent);

                if (currentCount <= 0) return;

                button.disabled = true;
                button.textContent = "…";

                try {
                    const { data, error } =
                        await supabaseClient.rpc(
                            "admin_remove_one_inventory_item",
                            {
                                target_user_id: adminSelectedUser.id,
                                target_item_name: itemName
                            }
                        );

                    if (error) throw error;

                    if (data !== true) {
                        alert("Unable to remove this item.");
                        return;
                    }

                    const newCount = currentCount - 1;

                    if (newCount <= 0) {
                        row.remove();
                    } else {
                        countElement.textContent = newCount;
                    }
                } catch (error) {
                    console.error("Admin −1 error:", error);
                    alert(
                        error.message ||
                        "Unable to remove item."
                    );
                } finally {
                    button.disabled = false;
                    button.textContent = "−";
                }
            });
        });
    }
})();

</script>

</body>
</html>
